  ...
  return (
    <section id="top" className="py-12 sm:py-16">
      ...
      <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mt-5 max-w-3xl text-lg text-slate-300">
        AcouLM connects a browser app shell, terminal CLI, and OpenAI-style HTTP API to local inference backends across CPU, GPU, and NPU.
      </motion.p>
      ...
      <div className="mt-8 flex flex-wrap gap-4">
        <a href="#quick-start" className="rounded-md bg-accent px-5 py-2.5 font-semibold text-slate-900 transition hover:brightness-110">Quick Start</a>
        <a href="https://github.com/est4ever/AcouLM" target="_blank" rel="noreferrer" className="rounded-md border border-accent/50 px-5 py-2.5 font-semibold text-accent transition hover:bg-accent/10">View GitHub</a>
      </div>
    </section>
  );
}
