...
bool BackendPool::load_device(const std::string& device, std::string& error_out) {
    if (backends.find(device) != backends.end()) {
        std::cout << "[BackendPool] Device " << device << " is already loaded.\n";
        return true;
    }
    if (model_path.empty()) {
        error_out = "No model is currently loaded in the pool; start with a model first.";
        std::cerr << "[BackendPool] Cannot hot-load device " << device << ": " << error_out << "\n";
        return false;
    }
    try {
        std::cout << "[BackendPool] Hot-loading model on " << device << "...\n";
        auto backend = std::make_unique<OpenVINOBackend>();
        backend->load_model(model_path, device);
        backends[device] = std::move(backend);
        std::cout << "[BackendPool] Hot-load complete: " << device << "\n";
        return true;
    } catch (const std::exception& e) {
        error_out = e.what();
        std::cerr << "[BackendPool] Hot-load failed for " << device << ": " << error_out << "\n";
        return false;
    }
}

void BackendPool::unload_device(const std::string& device) {
    if (device == current_device) {
        std::cerr << "[BackendPool] Cannot unload the currently active device: " << device << "\n";
        return;
    }
    backends.erase(device);
    std::cout << "[BackendPool] Unloaded device: " << device << "\n";
}
